...
from utils.vggt_dataset_v2 import Dataset, Parser, align_pose
...
from utils.my_strategy import MyStrategy as DefaultStrategy
...
class Config:
    ...
    eval_steps: List[int] = field(default_factory=lambda: [4_000, 10_000, 12_000])
    # Steps to save the model
    save_steps: List[int] = field(default_factory=lambda: [4_000, 10_000, 12_000])

    # Initialization strategy
    init_type: str = "sfm"
    # Initial number of GSs. Ignored if using sfm
    init_num_pts: int = 500000
...
def clean_splats(splats, optimizers=None, threshold=0.8):
    from sklearn.cluster import HDBSCAN
    
    # Get the original device
    device = splats["means"].device
    
    # Extract points and create valid mask for NaN/inf filtering
    points = splats["means"].detach().cpu().numpy()
    valid_mask = ~(np.isnan(points).any(axis=1) | np.isinf(points).any(axis=1))
    points_clean = points[valid_mask]
    
    if len(points_clean) == 0:
        print("Warning: No valid points found after filtering NaN/inf values")
        return splats, optimizers
    
    # Perform clustering
    hdb = HDBSCAN(min_cluster_size=1500)
    clustering = hdb.fit(points_clean)
    ...
    if len(unique_labels) == 0:
        print("Warning: No clusters found, keeping all points")
        return splats, optimizers
    ...
    # Select top threshold percent of clusters
    num_clusters_to_keep = max(1, int(len(cluster_sizes) * threshold))
    selected_clusters = [cluster_sizes[i][0] for i in range(num_clusters_to_keep)]
    
    print(f"Keeping {num_clusters_to_keep} out of {len(cluster_sizes)} clusters ({threshold*100:.1f}%)")
    
    # Create mask for points to keep
    cluster_mask = np.isin(labels, selected_clusters)
    
    # Map back to original indices (accounting for NaN/inf filtering)
    full_mask = np.zeros(len(points), dtype=bool)
    full_mask[valid_mask] = cluster_mask
    
    # Convert to torch tensor
    keep_mask = torch.from_numpy(full_mask).to(device)
    
    # Prune splats
    pruned_splats = torch.nn.ParameterDict()
    for key, param in splats.items():
        pruned_data = param.data[keep_mask]
        pruned_splats[key] = torch.nn.Parameter(pruned_data)
    
    # Prune optimizers if provided
    pruned_optimizers = None
    if optimizers is not None:
        pruned_optimizers = {}
        for name, optimizer in optimizers.items():
            if name in pruned_splats:
                # Create new optimizer with pruned parameters
                param_groups = []
                for group in optimizer.param_groups:
                    new_group = group.copy()
                    new_group['params'] = [pruned_splats[name]]
                    param_groups.append(new_group)
                
                # Create new optimizer of the same type
                optimizer_class = type(optimizer)
                pruned_optimizers[name] = optimizer_class(
                    param_groups,
                    **{k: v for k, v in optimizer.defaults.items() if k != 'params'}
                )
    
    print(f"Pruned from {len(splats['means'])} to {len(pruned_splats['means'])} splats")
    
    return pruned_splats, pruned_optimizers
...
def create_splats_with_optimizers(
    ...
    dist2_avg, _ =  knn(points, 4) # [N,]
    dist2_avg = (dist2_avg[:, 1:] ** 2).mean(dim=-1)
...
class Runner:
    ...
    def __init__(
        ...
        self.parser = Parser(
            ...
            use_global_alignment=True,  # Enable global alignment
            max_query_pts=4096,        # Optional: custom query points
            shared_camera=True,    # Optional: whether cameras are shared
            factor=cfg.data_factor,
            normalize = False,
            test_every = 10,
            results_dir=self.cfg.result_dir,
        )
        cfg.near_plane = self.parser.scene_scale * 0.05
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            if cfg.depth_loss:
                depths_gt = data["depth"].to(device)  # [1, M]
                disp =depths[:,:,:,0]  # [1, H, W]                
                disp_gt = depths_gt
                disp = (disp - disp.min()) / (disp.max() - disp.min())
                disp = torch.nan_to_num(disp, nan=0.0)
                disp_gt = (disp_gt - disp_gt.min()) / (disp_gt.max() - disp_gt.min())
                disp_gt = 1-disp_gt

                # print('disp --------------',torch.isnan(disp).any().item(), torch.isinf(disp).any().item())
                # print('disp_gt --------------',disp_gt.shape, torch.isnan(disp_gt).any().item(), torch.isinf(disp_gt).any().item())
                # Display disparity maps using matplotlib for debugging
                os.makedirs(os.path.join(cfg.result_dir, 'disparity_maps'), exist_ok=True)
                if step % 100 == 0 and world_rank == 0:
                    fig, axs = plt.subplots(2, 2, figsize=(10, 4))
                    axs[0, 0].imshow(disp[0].detach().cpu().numpy(), cmap='plasma')
                    axs[0, 0].set_title('Predicted Disparity')
                    axs[1, 0].imshow(disp_gt[0].detach().cpu().numpy(), cmap='plasma')
                    axs[1, 0].set_title('Ground Truth Disparity')
                    axs[0, 1].imshow(pixels[0].detach().cpu().numpy())
                    axs[0, 1].set_title('Input Image')
                    axs[1, 1].imshow(colors[0].detach().cpu().numpy())
                    axs[1, 1].set_title('Rendered Image')
                    plt.tight_layout()
                    plt.savefig(os.path.join(cfg.result_dir, 'disparity_maps', f"step_{step}.png"))
                    plt.close(fig)
                depthloss = F.l1_loss(disp, disp_gt) 
                # print('loss depth loss --------------',loss, depthloss)
                loss = depthloss * cfg.depth_lambda + (1 - cfg.depth_lambda) * loss
            if cfg.use_bilateral_grid:
            ...
            if not cfg.disable_viewer:
                ...
                # num_train_rays_per_sec = num_train_rays_per_sec
                self.viewer.render_tab_state.num_train_rays_per_sec = num_train_rays_per_sec
                # Update the scene.
                self.viewer.update(step, num_train_rays_per_step)
            ##########################################
            # if step % 1000 == 0 and step > 0:
            #     self.splats, self.optimizers = clean_splats(self.splats, self.optimizers)
    ...
    def eval(self, step: int, stage: str = "val", ckpt: dict = None):
        ...
        for i, data in enumerate(valloader):
            ...
            camtoworlds = transform @ camtoworlds
            Ks = data["K"].to(device)
            pixels = data["image"].to(device) / 255.0
            masks = data["mask"].to(device) if "mask" in data else None
            height, width = pixels.shape[1:3]
